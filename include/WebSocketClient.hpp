...
{
public:
	WebSocketClient(net::io_context& ioc, net::ssl::context& sslCtx, tcp::resolver& resolver,  std::string target, std::string& symbol) : ioc_{ioc}, sslCtx_{sslCtx}, resolver_{resolver}, ws_{std::make_unique<websocket::stream<net::ssl::stream<tcp::socket>>>(net::make_strand(ioc), sslCtx)}, target_{target}, symbol_{symbol}, interrupted_{true}, strand_{ioc.get_executor()} {}
